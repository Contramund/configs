...
myScratchPads = [ NS "Catia (L)" spawnCatiaL findCatiaL manageCatiaL,
  ...
  where
    ...
    manageCatiaL = customFloating $ W.RationalRect 0.02 0.05 0.47 0.87
    spawnCadenceR = "cadence"
    findCadenceR = title =? "Cadence"
    manageCadenceR = customFloating $ W.RationalRect 0.51 0.05 0.47 0.87
    spawnTerm = "alacritty -t scratchpad"
    findTerm = title =? "scratchpad"
    manageTerm = customFloating $ W.RationalRect 0.02 0.05 0.96 0.87
...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    ...
    -- addidional Keys
    --
    [ ((0                     , 0x1008ff11), spawn "pactl set-sink-volume 0 -5%"), -- amixer -q sset Master 2%-
      ((0                     , 0x1008ff13), spawn "pactl set-sink-volume 0 +5%"), -- amixer -q sset Master 2%+
